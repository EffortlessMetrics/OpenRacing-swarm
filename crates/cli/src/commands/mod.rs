...
pub enum HardwareCommands {
    ...
    SniffGuidedCapture {
        ...
        /// Observe-only hardware doctor receipt used to verify the USBPcap selector
        #[arg(long)]
        hardware_doctor: std::path::PathBuf,
        /// USBPcapCMD.exe path from hardware doctor
        #[arg(long)]
        usbpcapcmd: std::path::PathBuf,
        /// USBPcap control interface, for example \\.\USBPcap2
        ...
        #[arg(long)]
        out: std::path::PathBuf,
        /// Local operator notes Markdown file with 0x8E event marker fields
        #[arg(long)]
        operator_notes: std::path::PathBuf,
        /// Allow replacing an existing local pcapng at --out
        #[arg(long)]
        ...
        #[arg(long)]
        confirm_external_passive_capture: bool,
        /// Write the guided capture JSON receipt to this file
        #[arg(long)]
        json_out: Option<std::path::PathBuf>,
    },
...
pub enum MozaCommands {
    ...
    /// Review payload-bearing status-source candidate fixtures without sending traffic
    VendorStatusPayloadSourceSemanticReview {
        /// Stored payload-bearing status-source candidate receipt
        #[arg(long)]
        payload_source_candidates: std::path::PathBuf,
        /// Write the non-claiming payload source semantic review receipt to this JSON file
        #[arg(long)]
        json_out: std::path::PathBuf,
        /// Replace an existing payload source semantic review receipt
        #[arg(long)]
        overwrite: bool,
    },
