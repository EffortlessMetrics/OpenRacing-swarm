...
mod tests {
    ...
    #[test]
    fn parse_moza_vendor_status_authority_source_gap() -> TestResult {
        let cli = parse_cli([
            "wheelctl",
            "moza",
            "vendor-status-authority-source-gap",
            "--endpoint-candidates",
            "ci/hardware/moza-r5/2026-05-13/vendor-status-endpoint-candidates-from-payload-rerun.json",
            "--protocol-evidence-review",
            "ci/hardware/moza-r5/2026-05-13/vendor-protocol-evidence-review.json",
            "--json-out",
            "target/moza-current/vendor-status-authority-source-gap.json",
            "--overwrite",
        ])?;
        match &cli.command {
            Commands::Moza(MozaCommands::VendorStatusAuthoritySourceGap {
                endpoint_candidates,
                protocol_evidence_review,
                json_out,
                overwrite,
                ...
                assert_eq!(
                    json_out.to_str(),
                    Some("target/moza-current/vendor-status-authority-source-gap.json")
                );
                assert!(*overwrite);
            }
            _ => return Err("expected Moza VendorStatusAuthoritySourceGap command".into()),
        }
        Ok(())
    }
