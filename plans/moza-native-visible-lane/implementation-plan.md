...
## Work item: review-payload-source-semantic-fixtures

Status: completed
Linked proposal: docs/proposals/OR-PROP-0001-moza-native-visible-lane.md
Linked spec: docs/specs/OR-SPEC-0002-moza-r5-vendor-authority-test-lane.md
Linked ADR: docs/adr/0009-hardware-validation-evidence-state-machine.md
Blocks: timing-correlated status-source evidence or another reviewed authority-state source
Blocked by: reviewed payload-bearing authority-state source or equivalent timing-correlated capture

### Goal

Turn the four stored payload-bearing `0x8E` samples into fixture-backed
semantic review evidence while preserving the block on live probe,
authorization, PIDFF rerun, force escalation, and motion.

### Production delta

`wheelctl moza vendor-status-payload-source-semantic-review` reads only:

```text
vendor-status-payload-source-candidates.json
```

and writes `vendor-status-payload-source-semantic-review.json`.

The protocol fixture decoder now recognizes the four `0x8E` device-to-host
samples as the review group:

```text
passive_payload_bearing_status_source_0x8e
```

The review confirms fixture decoder coverage for:
...
The result is useful protocol navigation but insufficient authority-state
evidence:

```text
semantic_review_verdict=insufficient_for_authority_source
authority_state_source_verdict=blocked_missing_same_tuple_payload_variation_or_timing_correlation
same_tuple_payload_variation_observed=false
cross_tuple_payload_diversity_observed=true
...
- The payload-source semantic review consumes the payload-source candidate
  receipt and validates every `0x8E` sample through fixture decoder coverage.
- The samples remain `unknown_do_not_send` status-source questions only.
- The review records why the samples are not enough: no same-tuple payload
  variation and no timing correlation to authority or mode state.
- `payload_bearing_authority_state_source_found=false`,
  `live_read_only_probe_allowed=false`, `authorization_plan_allowed=false`, and
  `motion_attempt_allowed=false`.
- The next native-path action is timing-correlated `0x8E` evidence or another
  reviewed payload-bearing authority-state status source before any live probe,
  authorization, PIDFF rerun, force escalation, or motion attempt.

### Proof Commands

```powershell
cargo run --locked -p wheelctl --bin wheelctl -- --json moza vendor-status-payload-source-semantic-review `
  --payload-source-candidates ci/hardware/moza-r5/2026-05-13/vendor-status-payload-source-candidates.json `
  --json-out ci/hardware/moza-r5/2026-05-13/vendor-status-payload-source-semantic-review.json `
  --overwrite
python scripts/cargo_fmt_workspace.py
...
- the `vendor-status-payload-source-semantic-review` CLI, receipt, schema, and
  tests,
- the `0x8E` fixture review group in `response_semantics.rs`,
- `vendor-status-payload-source-semantic-review.json`,
- source-of-truth notes for this work item.

Do not remove the payload-source candidate receipt, response semantic fixture
receipt, response-source correlation receipt, source-gap receipt, endpoint
candidate receipts, targeted read-only payload rerun, status matrix, demux
...
## Work item: audit-vendor-status-movement-blocker

Status: completed
Linked proposal: docs/proposals/OR-PROP-0001-moza-native-visible-lane.md
Linked spec: docs/specs/OR-SPEC-0002-moza-r5-vendor-authority-test-lane.md
Linked ADR: docs/adr/0009-hardware-validation-evidence-state-machine.md
Blocks: native-visible-promotion

### Goal

Turn the read-only zero-response thread into one current blocker statement for
the movement lane, without authorizing another live probe or output attempt.

### Production delta

Add `wheelctl moza vendor-status-movement-blocker-audit` and the checked-in
receipt `vendor-status-movement-blocker-audit.json`. The audit consumes the
original zero-response status probe, the demux follow-up, the framing and
endpoint diagnoses, the payload-rerun diagnosis, and the 0x8E
timing-correlation plan/review.

The receipt records:

- the original `vendor-status-mode-matrix.json` decoded zero responses;
- the demux follow-up decoded seven non-authority status replies on the same
  serial lane;
- broad serial ownership, line-setting, framing, and scan-window depth are not
  the current primary blockers;
- authority endpoint/command mismatch and missing timing-correlated
  payload-bearing status-source evidence remain active blockers;
- read-only rerun, authorization, PIDFF rerun, force escalation, and motion are
  not allowed yet;
- the next concrete command path is the staged passive Pit House 0x8E
  event-marker capture and no-output timing review.

### Non-goals

No live hardware access, HID open, serial open, read-only query send, PIDFF
output, feature report, configuration write, firmware/update/DFU path, high
torque, mode-enable write, authority write, authorization receipt, semantic
decode claim, registry promotion, tuple sendability, corrected read-only probe
readiness, native-control claim, native-visible claim, smoke-ready claim,
simulator claim, coexistence claim, release-ready claim, or wheel movement.

### Acceptance

- The audit reads only stored receipts.
- It keeps `wheel_moved_under_openracing=false`,
  `visible_motion_verified=false`, `output_was_sent=false`, and
  `authority_state=blocked`.
- It names why the original zero-response probe is no longer an undiagnosed
  generic serial problem.
- It blocks another read-only rerun until a corrected registry-approved
  read-only endpoint or timing-correlated equivalent status source exists.
- It preserves the staged passive 0x8E event-marker capture as the next
...
cargo run --locked -p wheelctl --bin wheelctl -- --json moza artifact-index `
  --lane ci/hardware/moza-r5/2026-05-13 `
  --json-out target/moza-current/artifact-index-status-blocker-audit.json `
  --md-out ci/hardware/moza-r5/2026-05-13/index.md
python scripts/cargo_fmt_workspace.py
...
- the `vendor-status-movement-blocker-audit` CLI, receipt, schema, and tests;
- `vendor-status-movement-blocker-audit.json`;
- source-of-truth notes for this work item.

Do not remove the timing-correlation plan/review, status probe receipts,
...
## Work item: split-0x8e-marker-handoff-phases

Status: completed
Linked spec: docs/specs/OR-SPEC-0002-moza-r5-vendor-authority-test-lane.md
Linked ADR: docs/adr/0009-hardware-validation-evidence-state-machine.md
Blocks: operator confusion between pre-capture selector review markers and
capture-runtime event markers
Blocked by: completed 0x8E marker order alignment

### Goal

Make the generated 0x8E timing-correlation command handoff reflect the actual
operator phases: stamp the hardware-doctor selector review marker before
USBPcap capture starts, then stamp capture-start and semantic Pit House/KS LED
event markers while capture is running.

### Production delta

`wheelctl moza vendor-status-timing-correlation-plan` now emits two marker
handoff steps instead of one overloaded step:

- `stamp_hardware_doctor_selector_marker`, with
  `requires_capture_running=false`, `requires_capture_not_started=true`, and
  `command_example_marker=hardware_doctor_selector_reviewed_utc`.
- `stamp_capture_event_markers`, with `requires_capture_running=true` and
  `command_example_marker=capture_start_utc`.

Bench-wizard and artifact-index navigation preserve those phase fields from the
stored plan/audit receipts so the operator does not treat the selector-review
marker as something to stamp after capture starts.
...
- The timing-correlation plan and movement-blocker audit list a pre-capture
  `stamp_hardware_doctor_selector_marker` command before the passive capture
  command.
- The timing-correlation plan and bench wizard list a separate
  `stamp_capture_event_markers` command for `capture_start_utc` and the
  semantic capture-runtime markers.
- The pre-capture marker step records `requires_capture_running=false` and
  `requires_capture_not_started=true`.
- The capture-runtime marker step records `requires_capture_running=true` and
...
Restore only the previous single marker handoff template, focused tests,
regenerated plan/audit receipts, and this source-of-truth work item. Do not
remove the marker helper, strict timestamp validators, timing-correlation plan,
movement blocker audit, or native-visible promotion block.

## Work item: gate-0x8e-handoff-command-phases

Status: completed
Linked spec: docs/specs/OR-SPEC-0002-moza-r5-vendor-authority-test-lane.md
Linked ADR: docs/adr/0009-hardware-validation-evidence-state-machine.md
Blocks: operator confusion between immediate pre-capture commands, capture-runtime markers, and post-capture derived-evidence commands
Blocked by: completed 0x8E marker phase split

### Goal

Make the Moza bench wizard present only the currently runnable pre-capture setup
commands as "Next Operator Commands" while keeping capture-runtime and
post-capture commands visible with explicit prerequisite statuses.

### Production delta

`wheelctl moza bench-wizard` now classifies the 0x8E timing-correlation handoff
commands by phase:

- pre-capture setup and the hardware-doctor selector marker remain runnable now;
- the unrendered USBPcap capture template remains
  `placeholder_template_not_runnable`;
- capture event marker commands are marked `requires_capture_running`;
- sniff receipt, summary, bundle, and future timing review commands are marked
  `requires_completed_capture_artifacts`.

The markdown handoff says the "Next Operator Commands" table is immediate
pre-capture work only and surfaces the conditional commands in the external
capture command table with status.
...
- `stamp_hardware_doctor_selector_marker` remains an immediate pre-capture
  command and records `next_operator_command_status=runnable_before_capture`.
- `stamp_capture_event_markers` records `requires_capture_running=true` and is
  not listed as an immediate next-operator command.
- The placeholder USBPcap capture template remains non-runnable until
  `sniff-notes-template` renders the selector-bound command.
- Post-capture sniff receipt, summary, bundle, and timing review commands record
  completed-capture prerequisites and are not listed as immediate
...
Restore only the previous bench-wizard command-status classification, markdown
wording, tests, regenerated plan/audit receipts, and this source-of-truth work
item. Do not remove the marker helper, strict timestamp validators,
...
## Work item: bind-0x8e-review-to-sniff-capture-window

Status: completed
Linked proposal: docs/proposals/OR-PROP-0001-moza-native-visible-lane.md
Linked spec: docs/specs/OR-SPEC-0002-moza-r5-vendor-authority-test-lane.md
Linked ADR: docs/adr/0009-hardware-validation-evidence-state-machine.md
Blocks: event markers from outside the actual bounded USBPcap capture window
being accepted as future 0x8E timing-correlation candidate evidence
Blocked by: completed `vendor-status-timing-correlation-review` capture
provenance gate

### Goal

Bind the no-output 0x8E timing-correlation review to the actual
`wheelctl hardware sniff-capture` receipt window, not only the
operator-declared capture markers in notes.

### Production delta

`wheelctl moza vendor-status-timing-correlation-review` now requires verified
timing-correlation capture provenance to include:

```text
sniff_capture.started_at_utc
sniff_capture.duration_ms
sniff_capture.terminated_after_duration=true
```

The review derives the bounded capture stop time from that receipt, records the
window in `capture_provenance`, and emits a `capture_receipt_window` verdict.
Future timing candidates can only surface when all semantic operator markers
fit inside the sniff-capture receipt window with a small bounded tolerance.
Markers outside that receipt window produce
`insufficient_capture_receipt_window_mismatch` and keep the lane blocked.

This is bench plumbing for the current blocker: missing reviewed
timing-correlated payload-bearing authority/mode status evidence. It still
does not authorize a read-only probe, PIDFF rerun, output, or motion.
...
- Verified capture provenance records the sniff-capture start, derived stop,
  duration, and tolerance.
- Candidate timing evidence remains possible only when semantic operator event
  markers fit inside that bounded receipt window.
- Missing legacy capture window fields are rejected.
- Markers outside the sniff-capture receipt window produce a blocked
  mismatch verdict instead of candidate evidence.
- `wheel_moved_under_openracing=false`, `visible_motion_verified=false`,
  `output_was_sent=false`, and `authority_state=blocked` remain the operating
  state.
...
Remove only the sniff-capture receipt-window requirement, schema fields,
focused assertions, and this source-of-truth work item. Do not remove the
sniff-capture hardware-doctor freshness check, selector guard, local
capture-prep freshness gate, movement-blocker audit, timing-correlation
plan/review artifacts, or native-visible promotion block.
...
## Work item: expand-0x8e-capture-marker-command-list

Status: completed
Linked spec: docs/specs/OR-SPEC-0002-moza-r5-vendor-authority-test-lane.md
Linked ADR: docs/adr/0009-hardware-validation-evidence-state-machine.md
Blocks: relying on chat history or a single example marker for the 0x8E
capture-runtime event list
Blocked by: completed materialized 0x8E capture-marker handoff

### Goal

Make the 0x8E event-marker handoff executable from the machine-readable wizard
and plan receipts by listing every required runtime marker command, not only the
`capture_start_utc` example.

### Production delta

`wheelctl moza vendor-status-timing-correlation-plan` and the bench-wizard
materialized capture handoff now include:

- `required_capture_event_markers`
- `required_capture_event_marker_count=10`
- `marker_commands` for every capture-runtime event marker

The concrete runtime marker commands are:
...
Each generated command is a `wheelctl hardware sniff-marker` operator-notes edit
only. It opens no HID or serial path and sends no OpenRacing read-only, output,
configuration, firmware, DFU, PIDFF, or vendor command.

### Non-goals

No live capture, raw pcap commit, HID open, serial open, read-only query rerun,
PIDFF command, feature report, configuration write, firmware/DFU interaction,
high torque, mode write, authority write, authorization, semantic decode,
tuple sendability, native-control claim, native-visible claim, smoke-ready
claim, release-ready claim, or wheel-movement claim.
...
- `stamp_capture_event_markers` carries the full ordered runtime marker list and
  `required_capture_event_marker_count=10`.
- Every marker entry includes a concrete parseable `wheelctl hardware
  sniff-marker` command and keeps all OpenRacing hardware-output, HID, serial,
  read-only query, and write flags false.
- `vendor-status-timing-correlation-plan.json` and
  `vendor-status-movement-blocker-audit.json` preserve the full marker command
  list for the next passive Pit House 0x8E capture.
- The exact blocker remains the missing reviewed timing-correlated
  payload-bearing authority/mode status source.
- `wheel_moved_under_openracing=false`, `visible_motion_verified=false`,
  `output_was_sent=false`, and `authority_state=blocked` remain explicit.
...
Remove only the full marker command-list fields, focused assertions, regenerated
plan/audit receipts, and this source-of-truth work item. Do not remove the
