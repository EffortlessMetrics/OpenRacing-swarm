...
- **Moza vendor-specific control investigation** - six no-output sniff plans are recorded for Pit House, SimHub, and simulator sessions; Pit House open-idle and full-controls have non-claiming receipts/summaries, and the current protocol review ranks the highest-frequency unknown commanded `0x7E` tuples for semantic decode before any reviewed vendor-control plan
