...
- Those receipts remain below the 1 degree visible-motion threshold, so no visible-motion or smoke-ready success is claimed. The 2026-05-18 real 1 degree controlled-angle attempt is preserved as failed visible-motion evidence: it sent five bounded PIDFF writes, sent final Stop All, stayed post-stop stable, and timed out before target after about 0.181 degrees of movement. The reviewed `bounded-pidff-micro-step-v2` retry is preserved separately in `native-controlled-angle-retry-smoke.json`; it sent 33 bounded PIDFF writes, sent final Stop All, stayed post-stop stable, and remained in the same 0.181 degree response band. Attempt 03 is preserved separately in `native-controlled-angle-attempt-03-smoke.json`; it consumed `native-controlled-angle-attempt-03-authorization.json`, sent four `bounded-pidff-effect-lifecycle-v1` writes, sent final Stop All, stayed post-stop stable, and remained in the same 0.181 degree response band. The closed-loop attempt is preserved separately in `native-controlled-angle-closed-loop-smoke.json`; it consumed `native-controlled-angle-closed-loop-authorization.json`, recomputed bounded PIDFF force from live angle error, sent 672 bounded reports with zero write errors, sent final Stop All/final zero, stayed post-stop stable, and timed out before target at `angle_delta_degrees=0.13183794918745662`. The [controlled-angle analysis](moza-r5-controlled-angle-analysis.md), retry analysis, `native-controlled-angle-attempt-03-failure-analysis.json`, and `native-controlled-angle-closed-loop-failure-analysis.json` classify these as safe undertravel, and [PIDFF semantics diagnosis](moza-r5-pidff-semantics-diagnosis.md) records that repeated micro-step writes changed write count but not motion. The [PIDFF lifecycle trace](moza-r5-pidff-lifecycle-trace.md) is recorded in `native-pidff-lifecycle-trace.json`; it decodes the preserved command logs as repeated setup/start/Stop-All cycles. The [PIDFF effect lifecycle plan](moza-r5-pidff-effect-lifecycle-plan.md) is recorded in `native-pidff-effect-lifecycle-plan.json`; it names `bounded-pidff-effect-lifecycle-v1`, implemented for software preflight and attempt-specific exact authorization binding only. `native-pidff-standard-path-diagnosis.json` records the no-output architecture diagnosis and classifies `standard_pidff_controlled_angle_path_ineffective_in_current_r5_mode`. `native-controlled-angle-attempt-03-preflight.json` and `native-controlled-angle-closed-loop-preflight.json` record no-output dry-runs; they opened no HID device, sent no writes, and prove no motion. Attempt 03 and the closed-loop attempt authorize no further output. The consumed follow-up plan does not authorize 5 percent / 3000 ms, 5 percent / 30000 ms, 3 degree, 5 degree, 30 degree, or 90 degree right/left output; any future output family requires no-output Moza vendor-specific protocol evidence, a reviewed plan, separate authorization, and fresh command-bound bench-clear.
