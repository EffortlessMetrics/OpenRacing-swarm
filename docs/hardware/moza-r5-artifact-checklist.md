...
The same lane is not OpenRacing-control-ready and not smoke-ready complete: it still has no steering-angle stream proof, native actuator-profile smoke receipt, Pit House coexistence receipt, simulator telemetry proof, simulator FFB smoke receipt, OpenRacing-control verification/audit, smoke-ready verification, or smoke-ready audit. Pit House is not installed on the current bench, so coexistence remains an explicit later no-claim rather than an implicit prerequisite for native OpenRacing movement control.
...
| Native steering feedback | Future native steering stream proof command -> `steering-angle-stream-proof.json` | OpenRacing-control verifier requires same R5 endpoint, monotonic steering samples, angle units/baseline, and no output/feature/FFB/serial/firmware actions | Required gate exists; no checked-in receipt yet |
| Native actuator profile | Future native PIDFF actuator-profile smoke command -> `native-actuator-profile-smoke.json` | OpenRacing-control verifier requires same R5 endpoint, bounded PIDFF strategy, no direct `0x20`, no high torque, final Stop All cleanup, and no serial/firmware/DFU actions | Required gate exists; no checked-in receipt yet |
| OpenRacing control promotion | `wheelctl moza verify-bundle --stage openracing-control-ready` -> `openracing-control-verification.json`; `wheelctl moza promote-manifest --stage openracing-control-ready` -> `manifest-promotion-openracing-control.json` | `wheelctl moza audit-lane --stage openracing-control-ready`; manifest moves to `openracing_control_ready`, `hardware_validated=true`, `simulator_validated=false`, `release_ready=false` | Commands and gate exist; this stage deliberately excludes SimHub and Pit House compatibility receipts, but waits for native steering and actuator receipts |
