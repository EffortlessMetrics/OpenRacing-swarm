...
The repository contains the lane scaffolding, docs, manifest schema, CLI receipt producers, parser replay harness, service readiness overlay, verifier gates, and a dated real-hardware lane at `ci/hardware/moza-r5/2026-05-13/`. That lane has passed passive verification, zero-stage verification/audit, and staged init for Steven's R5 + KS/ES + SR-P + HBP stack, with descriptor trust, parser fixture validation, fixture promotion, observe-only status receipts, zero-torque proof, watchdog proof, bounded disconnect proof, `init-off.json`, and `init-standard.json` recorded.

The same lane is not smoke-ready complete: it has no low-torque receipt, Pit House coexistence receipt, simulator telemetry proof, simulator FFB smoke receipt, smoke-ready verification, or smoke-ready audit. Pit House is not installed on the current bench, so coexistence remains an explicit later no-claim rather than an implicit prerequisite for zero-stage completion.
...
| Keep research separate from validation | `docs/hardware/moza-validation-matrix.md` lists researched coverage as source/code verified only | Matrix row changes only after receipts and promotion | Matrix says `Zero proof ready; bounded FFB blocked`; simulator validation, high torque, and release readiness remain unclaimed |
...
| Low torque | `wheelctl moza torque-test --confirm-low-torque --max-percent 2` -> `low-torque-proof.json` | Generated guidance requires `--lane`, same-lane zero/init artifacts, and an explicit strategy. Direct `direct_report_0x20` requires trusted direct report `0x20` metadata and a same-lane direct zero proof; PIDFF `pidff_bounded_effect` requires same-lane PIDFF Stop All zero proof, descriptor-proven PIDFF Device Control metadata, descriptor-derived feature report length metadata for effect allocation, explicit effect setup/allocation proof, an R5-shaped Set Effect encoder, exact endpoint selector, bounded nonzero PIDFF writes, no direct `0x20`, and final Stop All cleanup. Current PIDFF command surface fails closed before HID because the bounded effect writer is not implemented. | Command and gate exist; no real low-torque receipt exists; live R5 V1 currently has PIDFF zero proof and direct `0x20` unavailable, so generated guidance must stay read-only |
...
| Pre-output readiness | `wheelctl moza pre-output-readiness` -> `pre-output-readiness.json` | Read-only ledger separates `ready_for_zero_torque` from `ready_for_ffb`, checks passive verification, passive audit, service/status no-output receipts, at least one implemented descriptor-trusted zero-output strategy, zero-stage receipts, and bounded-FFB prerequisites without opening HID devices or suggesting output while passive is red | Real `2026-05-13` readiness receipt reports `ready_for_zero_torque=true`, `ready_for_ffb=false`, PIDFF Device Control `0x0C` stop-all ready, direct `0x20` unavailable, and init / low-torque / Pit House / simulator blockers |
