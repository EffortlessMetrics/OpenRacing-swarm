...
| Low torque | `wheelctl moza torque-test --confirm-low-torque --max-percent 2` -> `low-torque-proof.json` | Generated guidance requires `--lane`, trusted direct report `0x20` metadata, and a same-lane direct-report zero proof before suggesting torque-test; actual low-torque preflight requires same-lane zero/init/descriptor artifacts before HID initialization; gate requires off/standard init summaries with timestamp/CRC matches, direct mode gate, bounded payload recomputation, abort-to-zero | Command and gate exist; no real low-torque receipt exists; live R5 V1 currently has PIDFF zero proof and direct `0x20` unavailable, so generated guidance must stay read-only |
