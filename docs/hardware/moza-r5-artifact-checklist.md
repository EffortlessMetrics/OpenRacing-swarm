...
| Native visible motion | `wheelctl moza actuator-visible-smoke --confirm-actuator-visible --max-percent 5` -> `native-actuator-visible-smoke.json`, or `wheelctl moza controlled-angle-smoke --target-degrees 1 --max-percent 5 --timeout-ms <=2000 --confirm-controlled-angle` after exact authorization -> `native-controlled-angle-smoke.json` | Native-visible verifier requires the same safety/provenance as response plus `success=true`, `movement_observed=true`, and measured delta meeting the visible-motion threshold. It does not require Pit House, SimHub, simulator telemetry, or simulator FFB | Real `2026-05-13` 5 percent / 2000 ms receipt still fails the visible-motion gate: `movement_observed=false`, `angle_delta_degrees=0.18127718013275285` below the 1 degree threshold. The follow-up plan records the consumed exact authorization and `planned_next_output.allowed=false`; 5 percent / 3000 ms, 5 percent / 30000 ms, and 90 degree right/left are not authorized. Generate the controlled-angle plan with `wheelctl moza receipt-template --kind controlled-angle-plan`, then use `wheelctl moza controlled-angle-smoke --dry-run --target-degrees 1 --max-percent 5 --timeout-ms 15000` as the no-output preflight. A real controlled-angle run remains blocked until `native-controlled-angle-authorization.json` matches the exact 1 degree command. No visible-motion or smoke-ready success is claimed |
