...
The same lane is not smoke-ready complete: the 1 percent native actuator-profile receipt proves the output rail and cleanup path, but the 5 percent native visible-motion receipt failed the measured movement threshold. Any later retry must first record a non-claiming `native-actuator-visible-follow-up-plan.json` review artifact, preserve the failed receipt, and require a fresh bench-clear for the exact next output command. Pit House coexistence, simulator telemetry, simulator FFB, smoke-ready promotion, and smoke-ready audit are also still missing. Pit House is not installed on the current bench, so coexistence remains an explicit later no-claim rather than an implicit prerequisite for native OpenRacing movement control.
