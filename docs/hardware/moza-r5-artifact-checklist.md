...
| Simulator telemetry | `wheelctl telemetry record` creates a recorder artifact; `wheelctl moza simulator-telemetry-proof` -> `simulator-telemetry-proof.json` | Telemetry gate requires telemetry-only operation, normalized snapshots, recorder provenance, an allowed live recording origin (`live_simhub_udp` for `simhub_bridge`, or a future live game adapter origin), no hardware output, and no faults | `--input` recordings and checked-in fixtures are explicitly rehearsal-only (`normalized_input`, `real_simulator_source=false`) and cannot pass the real simulator telemetry gate; no real simulator telemetry receipt exists |
