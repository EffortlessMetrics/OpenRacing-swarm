...
The same lane is not smoke-ready complete: the 1 percent native actuator-profile receipt proves the output rail and cleanup path, and both the preserved 5 percent / 2000 ms response-only receipt and the 2026-05-17 bounded-shaped PIDFF micro-profile receipt record a measurable 0.181277 degree steering response with successful writes and Stop All cleanup. Those receipts are actuator-response evidence, not visible-motion evidence, because they remain below the 1 degree visible-motion threshold.

Four controlled-angle receipts are now preserved as failed visible-motion evidence and profile-diagnosis input. The first 1 degree attempt sent five bounded PIDFF writes, the reviewed retry sent 33 bounded PIDFF writes, attempt 03 sent four bounded PIDFF effect-lifecycle writes, and the closed-loop attempt sent 672 bounded PIDFF writes recomputed from live steering-angle error. All four had zero write errors, final cleanup, and post-stop stability. The first three stayed in the same about 0.181277 degree response band; the closed-loop attempt timed out below target at `angle_delta_degrees=0.13183794918745662`. Treat these as safe undertravel receipts, not permission to rerun.

The non-claiming controlled-angle plan surface remains `native-controlled-angle-plan.json`. The closed-loop artifacts are `native-controlled-angle-closed-loop-preflight.json`, `native-controlled-angle-closed-loop-authorization.json`, `native-controlled-angle-closed-loop-smoke.json`, and `native-controlled-angle-closed-loop-failure-analysis.json`. No further attempt, force increase, dwell extension, or larger-angle target is authorized. `wheelctl moza authorize-visible-output` remains the no-output path for the older actuator-visible command, not the controlled-angle rerun path. Pit House coexistence, simulator telemetry, simulator FFB, smoke-ready promotion, and smoke-ready audit are also still missing. Pit House and SimHub are external compatibility work, not prerequisites for native OpenRacing movement control; `pit-house-availability.json` may honestly record that Pit House is absent, but it is not coexistence proof.

The broader no-output standard PIDFF path diagnosis is recorded in
`native-pidff-standard-path-diagnosis.json` and summarized in
[Moza R5 Standard PIDFF Path Diagnosis](moza-r5-standard-pidff-path-diagnosis.md).
It preserves all three controlled-angle receipts, keeps
`planned_next_output.allowed=false`, and identifies no-output Moza
vendor-specific enable/control path investigation as the next protocol research
branch before any future output family.

The closed-loop failure analysis preserves the feedback-bounded successor
attempt and reaches the same claim boundary: no native-visible promotion, no
rerun permission, and no future output family before no-output Moza
vendor-specific protocol evidence and a reviewed plan.

The vendor-authority rail is also now preserved through one consumed
`estop_set_ffb` frame and a comparable post-authority PIDFF response capture.
`vendor-post-authority-pidff-response.json` classifies the result as
`post_authority_pidff_response_regressed`: the baseline response was
`0.18127718013275285` degrees and the post-authority response was
`0.032959487296864154` degrees. This is evidence that the tested authority
frame did not unlock visible standard PIDFF authority; it does not authorize a
retry or promote native-visible readiness.

The vendor-control investigation has six passive sniff scenario plans under
`ci/hardware/sniff/moza-r5/2026-05-13`. Two Pit House scenarios now have
non-claiming receipts, summaries, and bundle manifests; the remaining scenarios
are still plan-only. None of these artifacts are native-visible evidence or
...
| Native visible motion | `wheelctl moza actuator-visible-smoke --confirm-actuator-visible --max-percent 5` -> `native-actuator-visible-smoke.json`, or `wheelctl moza controlled-angle-smoke --target-degrees 1 --max-percent 5 --timeout-ms <=2000 --confirm-controlled-angle` after `wheelctl moza authorize-controlled-angle-output` records exact authorization -> `native-controlled-angle-smoke.json`, retry output `native-controlled-angle-retry-smoke.json`, attempt-03 output `native-controlled-angle-attempt-03-smoke.json`, or closed-loop output `native-controlled-angle-closed-loop-smoke.json` | Native-visible verifier requires the same safety/provenance as response plus `success=true`, `movement_observed=true`, and measured delta meeting the visible-motion threshold. It does not require Pit House, SimHub, simulator telemetry, or simulator FFB | Real `2026-05-13` 5 percent / 2000 ms response receipts, the first 2026-05-18 real 1 degree controlled-angle attempt, the retry output `native-controlled-angle-retry-smoke.json`, `native-controlled-angle-attempt-03-smoke.json`, and `native-controlled-angle-closed-loop-smoke.json` still fail the visible-motion gate. The closed-loop receipt used `closed-loop-pidff-angle-v1`, wrote 672 bounded PIDFF reports with zero write errors, sent final Stop All/final zero, stayed post-stop stable, and timed out before target at `angle_delta_degrees=0.13183794918745662`. Their authorizations are consumed and no next output is authorized; 3 degree, 5 degree, 5 percent / 3000 ms, 5 percent / 30000 ms, 30 degree, and 90 degree right/left are not authorized. `native-pidff-semantics-diagnosis.json` records that repeated micro-step writes did not increase movement, `native-pidff-lifecycle-trace.json` records the decoded repeated setup/start/Stop-All lifecycle, `native-pidff-effect-lifecycle-plan.json` records the no-output profile plan, `native-controlled-angle-attempt-03-preflight.json` and `native-controlled-angle-closed-loop-preflight.json` record no-output dry-runs, and the failure analyses preserve safe-undertravel classifications. No visible-motion or smoke-ready success is claimed |
| Standard PIDFF path diagnosis | `native-pidff-standard-path-diagnosis.json`; [Moza R5 Standard PIDFF Path Diagnosis](moza-r5-standard-pidff-path-diagnosis.md) | Diagnosis-only artifact; it preserves the three controlled-angle receipts, forbids further blind PIDFF-family output, and keeps `planned_next_output.allowed=false` | Recorded as no-output architecture diagnosis. It concludes that transport writes, steering feedback, cleanup, and post-stop stability work, but standard PIDFF effect lifecycle does not produce visible controlled motion in the current R5 mode. The next path is no-output Moza vendor-specific enable/control investigation |
| Post-authority PIDFF response | `vendor-authority-attempt.json`; `vendor-post-authority-pidff-smoke.json`; `vendor-post-authority-pidff-response.json`; [Moza R5 Post-Authority PIDFF Response](moza-r5-post-authority-pidff-response.md) | Non-claiming comparison only; it validates the consumed authority attempt and comparable PIDFF receipts while keeping `native_control_evidence=false`, `hardware_output_authorized=false`, `native_visible_ready=false`, and `smoke_ready=false` | Recorded as a regression: baseline response `0.18127718013275285` degrees, post-authority response `0.032959487296864154` degrees, delta change `-0.1483176928358887`. No vendor-authority retry, PIDFF rerun, native-visible promotion, or smoke-ready promotion is authorized |
| Vendor-control passive sniff plans | `ci/hardware/sniff/moza-r5/2026-05-13/*/sniff-plan.json`; [Moza R5 Vendor-Control Investigation](moza-r5-vendor-control-investigation.md) | Protocol research artifacts; artifact-index requires matching receipts and summaries before a scenario is recorded | Six plans exist; `pit-house-open-idle` and `pit-house-full-controls` have non-claiming summaries, while four scenarios remain plan-only. They do not authorize output, do not satisfy native-visible or smoke-ready readiness, and do not include raw pcap data |
...
| Pre-output readiness | `wheelctl moza pre-output-readiness` -> `pre-output-readiness.json` | Read-only ledger separates `ready_for_zero_torque`, `ready_for_native_control`, `native_actuator_response_proven`, `native_visible_motion_proven`, `ready_for_external_compatibility`, and legacy `ready_for_ffb`; it checks passive verification, passive audit, service/status no-output receipts, descriptor-trusted zero-output strategy candidates, zero-stage receipts, OpenRacing control receipts, native response receipts, and external compatibility blockers without opening HID devices or suggesting output while passive is red. It also surfaces diagnostic `input_role_semantics`, `pit_house_compatibility`, and `simulator_compatibility` summaries without treating generic extended fields, Pit House, SimHub, or simulator artifacts as native-control prerequisites | Real `2026-05-13` readiness receipt reports `ready_for_zero_torque=true`, `ready_for_native_control=true`, `native_actuator_response_proven=true`, `native_visible_motion_proven=false`, `ready_for_ffb=false`, `ready_for_external_compatibility=false`, PIDFF Device Control `0x0C` stop-all ready, direct `0x20` unavailable, and native visible motion plus simulator telemetry still blocking later FFB/external-compatibility claims. It records `input_semantic_mapping_complete=false`, `semantic_status=partial_generic_aux_mapping`, six candidate-only R5 V1 extended slots, four ambiguous clutch/handbrake candidates, and no readiness claim from those candidates. It also records `pit_house_compatibility.readiness_claim=false`, `pit_house_compatibility.blocks_native_control=false`, `simulator_compatibility.readiness_claim=false`, and `simulator_compatibility.blocks_native_control=false` |
| Simulator telemetry | `wheelctl telemetry record --live-game --game dirt_rally_2` or `--live-simhub`; `wheelctl moza simulator-telemetry-proof` -> `simulator-telemetry-proof.json` | Telemetry gate requires telemetry-only operation, normalized snapshots, recorder provenance, an allowed live recording origin (`live_simhub_udp` for `simhub_bridge` or `live_game_adapter` for `real_game`), no hardware output, and no faults | `--input` recordings and checked-in fixtures remain explicitly rehearsal-only (`normalized_input`, `real_simulator_source=false`); the live-game path now exists, but no real simulator telemetry receipt has been recorded yet |
...
native-controlled-angle-attempt-03-failure-analysis.json
...
vendor-authority-authorization.json
vendor-authority-smoke-dry-run.json
vendor-authority-attempt.json
vendor-post-authority-pidff-smoke.json
vendor-post-authority-pidff-response.json
