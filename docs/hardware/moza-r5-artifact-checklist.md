...
The first vendor-control investigation step is recorded as five plan-only
passive sniff artifacts under `ci/hardware/sniff/moza-r5/2026-05-13`. They are
non-claiming `wheelctl hardware sniff-plan` artifacts for Pit House, SimHub, and
simulator scenarios. They are not pcap receipts, summaries, native-visible
evidence, or smoke-ready evidence.
...
| Vendor-control passive sniff plans | `ci/hardware/sniff/moza-r5/2026-05-13/*/sniff-plan.json`; [Moza R5 Vendor-Control Investigation](moza-r5-vendor-control-investigation.md) | Plan-only protocol research artifacts; artifact-index requires matching receipts and summaries before a scenario is recorded | Five plans exist and are `present_non_claiming`. They do not authorize output, do not satisfy native-visible or smoke-ready readiness, and do not include raw pcap data |
