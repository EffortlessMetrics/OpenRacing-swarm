...
| Native visible motion | `wheelctl moza actuator-visible-smoke --confirm-actuator-visible --max-percent 5` -> `native-actuator-visible-smoke.json`, or `wheelctl moza controlled-angle-smoke --target-degrees 1 --profile bounded-pidff-micro-step-v2 --max-percent 5 --timeout-ms <=2000 --confirm-controlled-angle` after `wheelctl moza authorize-controlled-angle-output` records exact authorization -> `native-controlled-angle-smoke.json` | Native-visible verifier requires the same safety/provenance as response plus `success=true`, `movement_observed=true`, and measured delta meeting the visible-motion threshold. It does not require Pit House, SimHub, simulator telemetry, or simulator FFB | Real `2026-05-13` 5 percent / 2000 ms response receipts and the 2026-05-18 real 1 degree controlled-angle attempt still fail the visible-motion gate: `movement_observed=false`, `angle_delta_degrees=0.18127718013275285` below the 1 degree threshold. The controlled-angle attempt wrote PIDFF successfully, sent final Stop All, stayed post-stop stable, and timed out before target. The authorization is consumed and no next output is authorized; 5 percent / 3000 ms, 5 percent / 30000 ms, and 90 degree right/left are not authorized. Preserve the receipt; `native-controlled-angle-plan.json` now records `bounded-pidff-micro-step-v2` as no-output retry-profile diagnosis, but no future controlled-angle output is authorized until a fresh exact authorization names that profile. No visible-motion or smoke-ready success is claimed |
