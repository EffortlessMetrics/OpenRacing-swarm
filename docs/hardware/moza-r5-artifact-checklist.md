...
| Native visible motion | `wheelctl moza actuator-visible-smoke --confirm-actuator-visible --max-percent 5` -> `native-actuator-visible-smoke.json`, or `wheelctl moza controlled-angle-smoke --target-degrees 1 --max-percent 5 --timeout-ms <=2000 --confirm-controlled-angle` after `wheelctl moza authorize-controlled-angle-output` records exact authorization -> `native-controlled-angle-smoke.json`, retry output `native-controlled-angle-retry-smoke.json`, or attempt-03 output `native-controlled-angle-attempt-03-smoke.json` | Native-visible verifier requires the same safety/provenance as response plus `success=true`, `movement_observed=true`, and measured delta meeting the visible-motion threshold. It does not require Pit House, SimHub, simulator telemetry, or simulator FFB | Real `2026-05-13` 5 percent / 2000 ms response receipts, the first 2026-05-18 real 1 degree controlled-angle attempt, the retry output `native-controlled-angle-retry-smoke.json`, and `native-controlled-angle-attempt-03-smoke.json` still fail the visible-motion gate: `movement_observed=false`, `angle_delta_degrees=0.18127718013275285` below the 1 degree threshold. The first, retry, and attempt-03 controlled-angle receipts wrote PIDFF successfully, sent final Stop All, stayed post-stop stable, and timed out before target. Their authorizations are consumed and no next output is authorized; 3 degree, 5 degree, 5 percent / 3000 ms, 5 percent / 30000 ms, 30 degree, and 90 degree right/left are not authorized. `native-pidff-semantics-diagnosis.json` records that repeated micro-step writes did not increase movement, `native-pidff-lifecycle-trace.json` records the decoded repeated setup/start/Stop-All lifecycle, `native-pidff-effect-lifecycle-plan.json` records the no-output profile plan, `native-controlled-angle-attempt-03-preflight.json` records the no-output dry-run for `bounded-pidff-effect-lifecycle-v1`, and `native-controlled-angle-attempt-03-failure-analysis.json` records the safe-undertravel classification after the consumed attempt. No visible-motion or smoke-ready success is claimed |
