...
The same lane is not smoke-ready complete: the 1 percent native actuator-profile receipt proves the output rail and cleanup path, and both the preserved 5 percent / 2000 ms response-only receipt and the 2026-05-17 bounded-shaped PIDFF micro-profile receipt record a measurable 0.181277 degree steering response with successful writes and Stop All cleanup. Those receipts are actuator-response evidence, not visible-motion evidence, because they remain below the 1 degree visible-motion threshold. Any later visible-motion attempt must first record a non-claiming `native-actuator-visible-follow-up-plan.json` review artifact, preserve the response receipt, record the intended next profile, and require a fresh bench-clear for the exact next output command. The visible-smoke command supports `constant-low-force` and `bounded-shaped-pidff-micro-profile` through `pidff-bounded-effect`, capped at 5 percent for up to 2000 ms; any higher or longer profile needs a separate software and safety plan. The checked-in follow-up plan records the consumed 2026-05-17 authorization, `planned_next_output.allowed=false`, a blocked 5 percent / 3000 ms retry, a rejected 5 percent / 30000 ms dwell, and a later 90 degree right/left controlled-angle target that requires staged smaller-angle receipts. Use `wheelctl moza receipt-template --kind controlled-angle-plan` to create the non-claiming `native-controlled-angle-plan.json` design artifact before authorizing that target. The controlled-angle writer is implemented, but actual writes require an exact `native-controlled-angle-authorization.json`; the first enabled rung is limited to 1 degree, 5 percent, and at most 2000 ms. `wheelctl moza authorize-visible-output` is the no-output path that preserves the current response-only receipt and records the exact command after fresh bench-clear, the CLI rejects actual visible-motion writes until that authorization exists, and after any actual visible-motion receipt is written the CLI consumes that authorization. Pit House coexistence, simulator telemetry, simulator FFB, smoke-ready promotion, and smoke-ready audit are also still missing. Pit House and SimHub are external compatibility work, not prerequisites for native OpenRacing movement control; `pit-house-availability.json` may honestly record that Pit House is absent, but it is not coexistence proof.
...
| Native visible motion | `wheelctl moza actuator-visible-smoke --confirm-actuator-visible --max-percent 5` -> `native-actuator-visible-smoke.json`, or `wheelctl moza controlled-angle-smoke --target-degrees 1 --max-percent 5 --timeout-ms <=2000 --confirm-controlled-angle` after `wheelctl moza authorize-controlled-angle-output` records exact authorization -> `native-controlled-angle-smoke.json` | Native-visible verifier requires the same safety/provenance as response plus `success=true`, `movement_observed=true`, and measured delta meeting the visible-motion threshold. It does not require Pit House, SimHub, simulator telemetry, or simulator FFB | Real `2026-05-13` 5 percent / 2000 ms receipt still fails the visible-motion gate: `movement_observed=false`, `angle_delta_degrees=0.18127718013275285` below the 1 degree threshold. The follow-up plan records the consumed exact authorization and `planned_next_output.allowed=false`; 5 percent / 3000 ms, 5 percent / 30000 ms, and 90 degree right/left are not authorized. Generate the controlled-angle plan with `wheelctl moza receipt-template --kind controlled-angle-plan`, then use `wheelctl moza controlled-angle-smoke --dry-run --target-degrees 1 --max-percent 5 --timeout-ms 15000` as the no-output preflight. `wheelctl moza authorize-controlled-angle-output` can then write the no-output `native-controlled-angle-authorization.json` for one exact 1 degree, 5 percent, <=2000 ms command after fresh bench-clear. No visible-motion or smoke-ready success is claimed |
...
| Pit House coexistence | `wheelctl moza pit-house-availability` -> optional `pit-house-availability.json`, then `wheelctl moza pit-house-evidence`, `wheelctl moza pit-house-observation`, `wheelctl moza pit-house-case`, and `wheelctl moza pit-house-proof` -> `pit-house-coexistence.json` plus process/window evidence and five case artifacts | Smoke-ready verifier requires all five cases, non-notes evidence artifacts, source receipt links, direct-mode block or ack, mode-mismatch fail-safe, firmware-page high-risk refusal. Availability is non-claiming and only prevents fabricated open-state guidance when Pit House is absent. Wheelctl-generated process/window snapshots must match the requested case before `pit-house-observation` accepts them. | Commands and gates exist; Pit House is not installed/running on the current bench, so no coexistence claim exists |
