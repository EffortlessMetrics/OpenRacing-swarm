...
| Passive input capture | `wheelctl moza capture-input` writes `captures/r5-idle.jsonl`, isolated through-R5 role captures, `captures/r5-aggregated-idle-after-controls.jsonl`, `captures/ks-controls.jsonl`, and `captures/es-controls.jsonl`; `wheelctl moza analyze-capture` can diagnose raw byte/word movement before assigning role semantics | `validate-captures` checks parser success, expected product IDs, role evidence movement, rim controls, per-line no-output assertions, and topology-declared optional direct-plug captures | Capture command and verifier exist; no real captures exist |
