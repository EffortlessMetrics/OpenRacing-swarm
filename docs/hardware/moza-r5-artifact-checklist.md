...
The same lane is not smoke-ready complete: the 1 percent native actuator-profile receipt proves the output rail and cleanup path, and the 5 percent / 2000 ms PIDFF receipt records a measurable 0.181277 degree steering response with successful writes and Stop All cleanup. That receipt is actuator-response evidence, not visible-motion evidence, because it remains below the 1 degree visible-motion threshold. Any later visible-motion attempt must first record a non-claiming `native-actuator-visible-follow-up-plan.json` review artifact, preserve the response receipt, record the intended next profile, and require a fresh bench-clear for the exact next output command. The current visible-smoke command supports only `constant-low-force`, `pidff-bounded-effect`, at up to 5 percent for up to 2000 ms; any higher, longer, or shaped profile needs a separate software and safety plan. Pit House coexistence, simulator telemetry, simulator FFB, smoke-ready promotion, and smoke-ready audit are also still missing. Pit House and SimHub are external compatibility work, not prerequisites for native OpenRacing movement control.
...
| Native response promotion | `wheelctl moza verify-bundle --stage native-response-ready` -> `native-response-verification.json`; `wheelctl moza promote-manifest --stage native-response-ready` -> `manifest-promotion-native-response.json` | `wheelctl moza audit-lane --stage native-response-ready`; manifest moves to `native_response_ready`, `hardware_validated=true`, `simulator_validated=false`, `release_ready=false` | Real `2026-05-13` response receipt is sufficient for this native stage once refreshed verification/promotion/audit receipts are generated |
...
| Pit House coexistence | `wheelctl moza pit-house-evidence`, `wheelctl moza pit-house-observation`, `wheelctl moza pit-house-case`, and `wheelctl moza pit-house-proof` -> `pit-house-coexistence.json` plus process/window evidence and five case artifacts | Smoke-ready verifier requires all five cases, non-notes evidence artifacts, source receipt links, direct-mode block or ack, mode-mismatch fail-safe, firmware-page high-risk refusal. Wheelctl-generated process/window snapshots must match the requested case before `pit-house-observation` accepts them. | Commands and gates exist; Pit House is not installed/running on the current bench, so no coexistence claim exists |
