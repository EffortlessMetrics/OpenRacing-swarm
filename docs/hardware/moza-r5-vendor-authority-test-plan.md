...
6. **Read-only vendor status probe**
   ...
   - Current evidence: `vendor-status-mode-matrix.json` records one live read-only probe on COM4 after a fresh hardware doctor. It sent nine registry-approved read-only queries and no output/configuration/firmware commands, but decoded zero responses, so mode/safety status remains unknown and still blocks authority planning.
   - Current diagnosis: `vendor-status-framing-diagnosis.json` classifies the stored readback as repeated `0x0E/0x71/0x05` ASCII `NRFloss`/`recvGap` diagnostic stream frames, including one desynchronized partial frame, rather than registry status/mode replies. The next blocker is transport framing, serial stream demultiplexing, or endpoint/command correlation.
