...
6. **Read-only vendor status probe**
   ...
   - Current extended-scan correlation: `vendor-status-extended-scan-targeted.json` repeats the same two-command read-only probe with `--max-response-frames-per-query 64`. It scans 19 frames, decodes zero authority-state replies, and `vendor-status-extended-scan-diagnosis.json` now classifies `7E00A1214D` as a checksum-valid zero-length response-like frame for `0xA1/0x21/no_command`, so shallow scan-window depth is no longer the immediate explanation and the current blocker is zero-length ACK/status reply correlation.
