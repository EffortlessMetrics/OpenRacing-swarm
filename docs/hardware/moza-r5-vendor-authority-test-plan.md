...
6. **Read-only vendor status probe**
   ...
   - Current timing-correlation review: `vendor-status-timing-correlation-review.json` reviews the existing accepted setting-change summary and notes, finds the four target `0x8E` samples as static/single-sample payloads, and keeps timing proof, live probe readiness, authorization, output, and motion blocked.
