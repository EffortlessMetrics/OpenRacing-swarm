...
6. **Read-only vendor status probe**
   ...
   - Current timing-correlation review: `vendor-status-timing-correlation-review.json` reviews the existing accepted setting-change summary and notes, finds timestamped same-tuple payload variation for the four target `0x8E` tuples, but keeps timing proof, live probe readiness, authorization, output, and motion blocked because the old notes lack semantic operator event markers. Administrative capture markers are chronology only and cannot surface a candidate correlation.
   - Current movement blocker audit: `vendor-status-movement-blocker-audit.json` consolidates the zero-response probe thread and records that broad serial ownership, line-setting, framing, and scan-window depth are not the current primary blockers; authority endpoint/command mismatch and missing timing-correlated payload-bearing status-source evidence still block live probes, authorization, output, and motion.
