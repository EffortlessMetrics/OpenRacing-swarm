...
6. **Read-only vendor status probe**
   ...
   - Current extended-scan correlation: `vendor-status-extended-scan-targeted.json` repeats the same two-command read-only probe with `--max-response-frames-per-query 64`. It scans 19 frames, decodes zero authority-state replies, and preserves the same `0xA1/0x21/0x4D` mismatch, so shallow scan-window depth is no longer the immediate explanation for the missing authority-state replies.
