...
their tuples are unknown. It also preserves repeated packet-order hints for the
`0x5A/0x1B/0x00` then `0x5D/0x1B/0x01` pair and the
`0x25/0x19/0x02`, `0x25/0x19/0x03`, `0x25/0x19/0x01` triad, plus payload-shape
hints showing empty `0x5A` samples and zero-filled `0000` payloads for the
`0x5D` and `0x25` samples. Two data-length packets still lack extracted payload
bytes. This is protocol-shape, registry-coverage, frequency-prioritization,
sequence-shape, payload-shape, and sample fixture navigation only, not
