...
| Read-only vendor status/mode matrix recorded | `vendor-status-mode-matrix-hardware-doctor.json`; `vendor-status-mode-matrix.json`; `vendor-status-mode-matrix-plan.json` | Recorded failed-closed read-only evidence | The fresh doctor saw R5 `0x346E:0x0004` on COM4 and Pit House not running. The guarded `vendor-status-probe` sent nine registry-approved read-only status queries, opened no HID path, and sent no output/configuration/firmware commands. It decoded zero responses and recorded nine failed responses, so `real_hardware_status_evidence=false` and unknown mode/safety status still block authority planning. |
| Read-only serial framing diagnosis recorded | `vendor-status-framing-diagnosis.json`; `schemas/moza-vendor-status-framing-diagnosis.schema.json` | Pass as no-output diagnosis | The stored #73 readback did capture response frames, but not registry status/mode replies. Eight frames classify as valid `0x0E/0x71/0x05` ASCII `NRFloss`/`recvGap` diagnostic stream frames and one as a desynchronized partial frame. The receipt keeps `wheel_moved_under_openracing=false`, `visible_motion_verified=false`, `output_was_sent=false`, and `authority_state=blocked`; the next blocker is transport framing, serial stream demultiplexing, or endpoint/command correlation. |
| Read-only zero-length authority-status correlation recorded | `vendor-status-extended-scan-targeted.json`; `vendor-status-extended-scan-diagnosis.json`; `vendor-status-ack-only-correlation-targeted.json`; `vendor-status-ack-only-correlation-diagnosis.json`; `schemas/moza-vendor-status-framing-diagnosis.schema.json` | Pass as no-output/read-only diagnosis | The stored #77 extended scan and fresh ACK-only rerun both decoded zero authority-state replies. The regenerated diagnosis classifies `7E00A1214D` as a checksum-valid ACK-only/no-payload response-like frame for `0xA1/0x21/no_command`, not a command mismatch or status payload. It remains correlation evidence only: no semantic decode, registry promotion, tuple sendability, authorization, output, native-control, or native-visible claim. |
| Payload-bearing status-source candidates recorded | `vendor-status-payload-source-candidates.json`; `schemas/moza-vendor-status-payload-source-candidates.schema.json` | Pass as no-output passive-source candidate review | The receipt preserves four nonzero device-to-host `0x8E` setting-change samples (`0x8E/0x21/0x00`, `0x8E/0x31/0x00`, `0x8E/0x71/0x00`, and `0x8E/0x91/0x00`) as `unknown_do_not_send` status-source questions. It records no same-tuple payload variation, no semantic decode, no registry promotion, no live probe readiness, no authorization, no output, no native-visible claim, and keeps `authority_state=blocked`. |
| Payload-source semantic review recorded | `vendor-status-payload-source-semantic-review.json`; `schemas/moza-vendor-status-payload-source-semantic-review.schema.json`; `crates/hid-moza-protocol/src/serial/response_semantics.rs` | Pass as no-output fixture-backed semantic review | The fixture decoder recognizes the four `0x8E` samples as `passive_payload_bearing_status_source_0x8e` and preserves their nonzero payloads. The review still classifies them as insufficient for authority planning because they are setting-change-only, show no same-tuple payload variation, and lack timing correlation to authority or mode state. It keeps `payload_bearing_authority_state_source_found=false`, `live_read_only_probe_allowed=false`, `authorization_plan_allowed=false`, `motion_attempt_allowed=false`, `wheel_moved_under_openracing=false`, `visible_motion_verified=false`, `output_was_sent=false`, and `authority_state=blocked`. |
| 0x8E timing-correlation plan recorded | `vendor-status-timing-correlation-plan.json`; `schemas/moza-vendor-status-timing-correlation-plan.schema.json` | Pass as no-output capture planning | The receipt consumes the payload-source semantic review and stages a passive Pit House event-marker capture for the same four `0x8E` samples. It requires a fresh hardware doctor selector, `sniff-capture --hardware-doctor`, KS top-left front LED default-teal/red/default-teal markers, complete operator notes, and local-only raw pcap handling. It records `capture_recorded=false`, `timing_correlation_proven=false`, `live_read_only_probe_allowed=false`, `authorization_plan_allowed=false`, `motion_attempt_allowed=false`, `wheel_moved_under_openracing=false`, `visible_motion_verified=false`, `output_was_sent=false`, and `authority_state=blocked`. |
