...
The PIDFF low-torque strategy is explicit and verifier-distinct. Its current software surface validates the same-lane PIDFF Stop All zero proof, off/standard init receipts, exact lane endpoint selector, and descriptor-proven PIDFF Device Control report, then fails closed before opening HID because a bounded PIDFF effect writer is not implemented yet:
...
- PIDFF strategy: no direct torque report `0x20`; bounded PIDFF effect writes only after a separate effect encoder is implemented.
