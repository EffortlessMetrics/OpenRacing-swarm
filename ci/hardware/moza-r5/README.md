...
Native actuator response and native visible motion are staged separately after the OpenRacing control foundation. A small 5 percent R5 PIDFF command can produce a sub-degree response; that proves the native output path is alive when it clears the response noise floor, but it does not prove operator-visible motion. The live 2026-05-13 receipt recorded about 0.181 degrees over 2000 ms, successful PIDFF writes, and Stop All cleanup, so it is response evidence while remaining non-claiming for visible motion:
...
The 1% actuator-profile receipt proves the native PIDFF output rail and cleanup path, but it does not claim visible motion. The 5% / 2000 ms live receipt should be treated as actuator-response evidence because it measured about 0.181 degrees of steering delta, not as a failed output path. Before `native-visible-ready` or smoke-ready promotion, collect a separate visible-motion receipt or reviewed follow-up profile that proves operator-visible steering delta from the R5 input stream while keeping final PIDFF Stop All cleanup and all high-risk surfaces disabled:
