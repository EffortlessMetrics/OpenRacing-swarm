...
Today this PIDFF command validates same-lane PIDFF Stop All zero proof, off/standard init receipts, the exact lane endpoint, and descriptor-proven PIDFF Device Control metadata, then fails closed before opening HID because the bounded PIDFF effect writer is not implemented. A future PIDFF receipt must declare `low_torque_strategy: "pidff_bounded_effect"`, record bounded nonzero PIDFF writes, avoid direct report `0x20`, and end with PIDFF Stop All cleanup. It cannot satisfy the direct-report verifier path.
