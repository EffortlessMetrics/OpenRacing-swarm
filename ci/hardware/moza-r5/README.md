...
- Low torque requires `--confirm-low-torque`, `--lane`, same-lane real zero/off/standard init receipts before HID initialization, trusted descriptor-derived direct report `0x20` metadata, and a same-lane direct-report zero proof accepted by `torque-test`. Explicit `--zero-proof`, `--init-off`, `--init-standard`, and `--descriptor` paths are accepted only when they resolve to the expected dated-lane artifacts. The verifier re-reads the same dated lane's zero/off/standard prerequisite receipts, checks their embedded timestamp/CRC summaries, and recomputes raw direct-torque payloads from the R5 PID and claimed percent before accepting the command log.
...
- If direct report `0x20` remains unproven, generated next-commands must stop at read-only readiness guidance. `--explicit-operator-override` is a separate deliberate operator decision for a later manual path, never generated guidance, and high torque remains disabled.
