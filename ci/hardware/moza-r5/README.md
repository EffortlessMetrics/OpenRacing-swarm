...
For broader external-app protocol research, use the passive USB sniffing evidence model in `docs/hardware/usb-sniffing.md` and scenario taxonomy in `docs/hardware/sniffing-scenarios.md`. Those artifacts may document host-side USB URBs from Pit House, SimHub, simulators, bridges, or the OS, but they are not Moza lane readiness receipts: they do not satisfy native response, native visible motion, smoke-ready, or release-ready gates, and raw `.pcapng` captures are not committed by default. The same safety boundary applies: no Zadig, driver replacement, WinUSB conversion, OpenRacing output commands, OpenRacing HID feature reports, serial config, firmware, or DFU.
...
Native actuator response and native visible motion are staged separately after the OpenRacing control foundation. A small 5 percent R5 PIDFF command can produce a sub-degree response; that proves the native output path is alive when it clears the response noise floor, but it does not prove operator-visible motion. The live 2026-05-13 lane preserves the original response-only receipt and the 2026-05-17 bounded-shaped PIDFF micro-profile receipt; both measured about 0.181 degrees over 2000 ms with successful PIDFF writes and Stop All cleanup. They are response evidence while remaining non-claiming for visible motion:

```powershell
wheelctl moza verify-bundle --lane ci/hardware/moza-r5/YYYY-MM-DD --stage native-response-ready --json-out ci/hardware/moza-r5/YYYY-MM-DD/native-response-verification.json
wheelctl moza promote-manifest --lane ci/hardware/moza-r5/YYYY-MM-DD --stage native-response-ready --json-out ci/hardware/moza-r5/YYYY-MM-DD/manifest-promotion-native-response.json
wheelctl moza audit-lane --lane ci/hardware/moza-r5/YYYY-MM-DD --stage native-response-ready --json-out ci/hardware/moza-r5/YYYY-MM-DD/lane-audit-native-response.json
wheelctl moza verify-bundle --lane ci/hardware/moza-r5/YYYY-MM-DD --stage native-visible-ready --json-out ci/hardware/moza-r5/YYYY-MM-DD/native-visible-verification.json
```

Only promote `native-visible-ready` after an intentionally reviewed visible-motion profile passes. Pit House, SimHub, simulator telemetry, and simulator FFB remain external compatibility gates and do not block native response or native visible-motion bring-up.
...
wheelctl moza pit-house-availability --operator Steven --evidence "Pit House install/process/window availability snapshot." --json-out ci/hardware/moza-r5/YYYY-MM-DD/pit-house-availability.json
wheelctl moza pit-house-evidence --case closed --operator Steven --evidence "Pit House closed before staged handshake." --require-match --json-out ci/hardware/moza-r5/YYYY-MM-DD/pit-house-evidence-closed.json
wheelctl moza pit-house-evidence --case open-standard --operator Steven --evidence "Pit House open and idle while standard mode completed." --require-match --json-out ci/hardware/moza-r5/YYYY-MM-DD/pit-house-evidence-open-standard.json
wheelctl moza pit-house-evidence --case open-direct --operator Steven --evidence "Pit House open while direct mode was blocked or required acknowledgement." --require-match --json-out ci/hardware/moza-r5/YYYY-MM-DD/pit-house-evidence-open-direct.json
wheelctl moza pit-house-evidence --case mode-change --operator Steven --evidence "Pit House mode change observed during bounded run; output cleared." --require-match --json-out ci/hardware/moza-r5/YYYY-MM-DD/pit-house-evidence-mode-change.json
wheelctl moza pit-house-evidence --case firmware-page --operator Steven --evidence "Pit House firmware/update page open; high-risk tests refused." --require-match --json-out ci/hardware/moza-r5/YYYY-MM-DD/pit-house-evidence-firmware-page.json
...
The 1% actuator-profile receipt proves the native PIDFF output rail and cleanup path, but it does not claim visible motion. The preserved 5% / 2000 ms response receipt and the 2026-05-17 5% / 2000 ms bounded-shaped micro-profile receipt should be treated as actuator-response evidence because they each measured about 0.181 degrees of steering delta, not as failed output paths. Before `native-visible-ready` or smoke-ready promotion, collect a separate visible-motion receipt or reviewed follow-up profile that proves operator-visible steering delta from the R5 input stream while keeping final PIDFF Stop All cleanup and all high-risk surfaces disabled:
...
If a real response receipt exists but does not satisfy the visible-motion threshold, do not rerun, raise force, or replace `native-actuator-visible-smoke.json` from generated guidance. Create a non-claiming follow-up plan first, preserve the response receipt, inspect bench/setup/FFB mode, and record the intended next profile before any output command is authorized. The visible-smoke command supports `constant-low-force` and `bounded-shaped-pidff-micro-profile` through `pidff-bounded-effect`, capped at 5 percent for up to 2000 ms; any higher or longer profile needs a separate software and safety plan plus a fresh bench-clear for the exact command. The 2026-05-17 exact authorization has been consumed, so `planned_next_output.allowed=false` again and actual visible-motion writes are rejected at preflight. A 5% / 3000 ms retry, a 5% / 30000 ms dwell, and a 90 degree right/left reset are not authorized by the current plan. The controlled-angle writer now exists as a feedback-bounded path that stops from steering samples, returns to the start angle, and is staged through smaller angle receipts first. The 2026-05-18 real 1 degree controlled-angle attempt is preserved as `native-controlled-angle-smoke.json`: it sent five bounded PIDFF writes, sent final Stop All, stayed post-stop stable, and timed out before target after about 0.181 degrees of movement. That receipt is evidence for profile diagnosis, not a rerun trigger; see [Moza R5 controlled-angle analysis](../../../docs/hardware/moza-r5-controlled-angle-analysis.md). The reviewed `bounded-pidff-micro-step-v2` retry used `native-controlled-angle-retry-preflight.json`, `native-controlled-angle-retry-authorization.json`, and `native-controlled-angle-retry-smoke.json`; it increased bounded PIDFF writes to 33 but stayed in the same 0.181 degree response band with final Stop All and stable post-stop behavior. The lifecycle trace is recorded as `native-pidff-lifecycle-trace.json`: it classifies the retry sequence as repeated setup/start/Stop-All cycles and confirms the first attempt and retry stayed in the same delta band. `ci/hardware/moza-r5/2026-05-13/native-pidff-effect-lifecycle-plan.json` and [docs/hardware/moza-r5-pidff-effect-lifecycle-plan.md](../../../docs/hardware/moza-r5-pidff-effect-lifecycle-plan.md) now name `bounded-pidff-effect-lifecycle-v1`, implemented for software preflight and attempt-specific exact authorization binding only. `native-controlled-angle-attempt-03-preflight.json` records the no-output dry-run for that profile: it opened no HID device, sent no writes, and proves no motion. The current frontier remains `repeated_safe_undertravel`: next output allowed is false, native-visible remains false, smoke-ready remains false, and any attempt-03 output still requires fresh command-bound bench-clear plus exact authorization.
...
wheelctl moza pidff-lifecycle-trace --lane ci/hardware/moza-r5/YYYY-MM-DD --receipt ci/hardware/moza-r5/YYYY-MM-DD/native-controlled-angle-retry-smoke.json --json-out ci/hardware/moza-r5/YYYY-MM-DD/native-pidff-lifecycle-trace.json --md-out target/moza-pidff-lifecycle-trace.md
