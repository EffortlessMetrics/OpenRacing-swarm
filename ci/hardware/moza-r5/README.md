...
If a real visible-motion receipt exists but fails the measured movement threshold, do not rerun, raise force, or replace `native-actuator-visible-smoke.json` from generated guidance. Create a non-claiming follow-up plan first, preserve the failed receipt, inspect bench/setup/FFB mode, and require a fresh bench-clear for the exact next output command:
