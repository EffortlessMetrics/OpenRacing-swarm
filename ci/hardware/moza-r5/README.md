...
wheelctl moza torque-test --device <r5> --lane ci/hardware/moza-r5/YYYY-MM-DD --strategy direct-report-0x20 --zero-proof ci/hardware/moza-r5/YYYY-MM-DD/zero-torque-proof.json --descriptor ci/hardware/moza-r5/YYYY-MM-DD/descriptor.json --confirm-low-torque --max-percent 2 --duration-ms 250 --json-out ci/hardware/moza-r5/YYYY-MM-DD/low-torque-proof.json
...
Today this PIDFF command validates same-lane PIDFF Stop All zero proof, off/standard init receipts, the exact lane endpoint, and descriptor-proven PIDFF Device Control metadata, then fails closed before opening HID because the bounded PIDFF effect writer is not implemented. A future PIDFF path also needs descriptor-derived feature report length metadata for effect allocation (`0x11`/`0x12`/`0x13`) and an R5-shaped Set Effect encoder; the live R5 V1 descriptor exposes report `0x01` with a non-generic length, so generic PIDFF packet assumptions are not enough. A future PIDFF receipt must declare `low_torque_strategy: "pidff_bounded_effect"`, prove effect setup/allocation explicitly, record bounded nonzero PIDFF writes, avoid direct report `0x20`, and end with PIDFF Stop All cleanup. It cannot satisfy the direct-report verifier path.
...
- If direct report `0x20` remains unproven and the PIDFF bounded-effect writer is not implemented, generated next-commands must stop at read-only readiness guidance. `--explicit-operator-override` is a separate deliberate operator decision for a later manual path, never generated guidance, and high torque remains disabled.
